...
def send_contact_email_async(context_dict):
    # send_mail(
    #     to_email=["marketing@advancedentalexport.com"],  # company marketing email
...
@csrf_exempt
def save_lead_internal(request):
    """
    Super-fast internal sync endpoint for Cloud Run.
    Takes ~2 milliseconds. Only writes to db.sqlite3.
    NO CRM calls, NO SMTP email blocks on GoDaddy.
    Handles both new submissions, updates, and cancellations (sets is_cancel=True).
    """
    import json
    if request.method != 'POST':
    ...
    auth_header = request.headers.get('X-Internal-Secret') or request.META.get('HTTP_X_INTERNAL_SECRET')
    expected_secret = getattr(settings, 'INTERNAL_SYNC_SECRET', 'usd-secret-sync-token-2026')
    if auth_header != expected_secret:
        print(f"[WARN] save_lead_internal: Unauthorized attempt (received: {auth_header})")
    ...
    try:
        ...
        # 3. Handle Cancellation & Lead ID
        lead_id = data.get('lead_id') or data.get('id') or data.get('submission_id')
        is_cancel = bool(data.get('is_cancel') in [True, 'true', 'True', 1, '1'])
        is_update = bool(data.get('is_update') in [True, 'true', 'True', 1, '1'])
        phone_val = str(data.get('phone', '')).strip()
        
        email_val = data.get('email')
        if not email_val or '@' not in str(email_val):
            email_val = 'no-reply@ultimatesmiledesign.com'
        ...
        # If no lead_id found, match the most recent lead with same phone ONLY if this is an update or cancellation
        ...
        if lead:
            if 'is_cancel' in data and hasattr(lead, 'is_cancel'):
            ...
            if data.get('doctor_name'):
                lead.doctor_name = data.get('doctor_name')
            if data.get('message'):
                lead.message = data.get('message')
            lead.save()
            print(f"[SUCCESS] save_lead_internal: Updated lead ID {lead.id} (Doctor: {lead.doctor_name}, is_cancel={getattr(lead, 'is_cancel', is_cancel)})")
            return JsonResponse({'status': 'success', 'lead_id': lead.id, 'is_cancel': getattr(lead, 'is_cancel', is_cancel)})
        else:
            create_kwargs = {
            ...
            if hasattr(UserSubmission, 'is_cancel'):
                create_kwargs['is_cancel'] = is_cancel
            lead = UserSubmission.objects.create(**create_kwargs)
            print(f"[SUCCESS] save_lead_internal: Successfully saved lead ID {lead.id} ({lead.first_name} {lead.last_name}, Phone: {lead.phone}, is_cancel={is_cancel}) to UserSubmission!")
            return JsonResponse({'status': 'success', 'lead_id': lead.id, 'is_cancel': is_cancel})

    except Exception as e:
        print(f"[ERROR] save_lead_internal failed: {e}")

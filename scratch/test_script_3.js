    ...
    /* dj tag */ const WS_URL = 'wss://voice-agent-ws-705379243140.asia-southeast1.run.app/ws/voice-agent/'; /* dj tag */
    ...
    function checkBotFinishedSpeaking() {
        // ⚡ Ensure state transitions back to listening as soon as playback finishes
    ...
    function extractUserName() {
        ...
        for (let i = 0; i < chatHistory.length; i++) {
            ...
            if (turn.role === 'user' && turn.parts && turn.parts[0] && turn.parts[0].text) {
                ...
                let expMatch = text.match(/(?:my name is|myself|call me|mera naam|maru naam|i am|i'm|this is|hu)\s+([A-Za-z]+)(?:\s+([A-Za-z]+))?/i);
                ...
                if (prevBotAskedName || i === 0 || i === 1) {
                    const bareMatch = text.match(/^([A-Za-z]{2,20})(?:\s+([A-Za-z]{2,20}))?$/i);
    ...
    function extractUserCity() {
        ...
        for (const [dKey, cVal] of Object.entries(docCityMap)) {
            if (uDoc.includes(dKey)) return cVal;
        }
        return "";
    }
    ...
    function extractUserDoctor() {
        ...
        if (existing && existing.trim()) return existing.trim();
        const savedSlots = JSON.parse(sessionStorage.getItem('usd_booking_slots') || '{}');
        if (savedSlots.doctor_name) return savedSlots.doctor_name;
        ...
        try {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            console.error("Direct recorder failed:", err2);
                        }
                    }
            ...
            socket.onmessage = async (event) => {
                ...
                if (isVoiceMode && data.turnId !== undefined) {
                    ...
                    } else if (data.turnId < localTurnId) {
                        return;
                    } else if (data.turnId > localTurnId) {
                        localTurnId = data.turnId;
                    }
    ...
    async function submitChatData(ratingLabel = "Call Ended") {
        ...
        const log = document.getElementById('usd-chat-log');
        if (log) {
            const msgs = log.querySelectorAll('.user_message, .bot_message, .msg');
            msgs.forEach(m => {
                const isUser = m.classList.contains('user_message');
                const isBot = m.classList.contains('bot_message');
                const p = m.querySelector('.usd-msg-text');
                const txt = p ? p.innerText.trim() : m.innerText.trim();
                if (txt && !txt.includes("Riya is typing") && !txt.includes("Riya is thinking") && !txt.includes("Listening...")) {
        ...
        formData.append('rating', ratingLabel);
        ...
        if (window.recordedAudioBlob && window.recordedAudioBlob.size > 0) {
            ...
            formData.append('audio', window.recordedAudioBlob, `voice_recording.${ext}`);
        ...
        try {
            const resp = await fetch('/voice-agent/api/submit-feedback/', {
                method: 'POST',
                body: formData
            });
            const resJson = await resp.json();
            console.log("[INFO] Transcript and audio submitted successfully to marketing email:", resJson);
            window.isChatFeedbackSubmitted = true;
        } catch (err) {
            console.error("[ERROR] Failed to submit transcript to marketing email:", err);
        } finally {
            window.isSubmittingChatData = false;
        }
    }

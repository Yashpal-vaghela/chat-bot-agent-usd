 ...
 {% comment %} const WS_URL = 'wss://usd-chat-bot-705379243140.asia-southeast1.run.app/ws/voice-agent/';{% endcomment %}
  ...
  async function submitChatData(ratingLabel = "Call Ended") {
      ...
      try {
          ...
          console.warn("[WARN] Origin submit-feedback failed, trying Cloud Run:", localErr);
      }

      if (!submittedSuccessfully && typeof CLOUD_RUN_URL !== 'undefined' && CLOUD_RUN_URL) {
          try {
              const resp = await fetch(`${CLOUD_RUN_URL}/voice-agent/api/submit-feedback/`, {
                  method: 'POST',
                  body: JSON.stringify(payload),
                  headers: { 'Content-Type': 'application/json' }
              });
              if (resp.ok) {
                  const resJson = await resp.json();
                  console.log("[INFO] Transcript and audio submitted successfully to marketing email (via Cloud Run):", resJson);
                  submittedSuccessfully = true;
              }
          } catch (cloudErr) {
              console.error("[ERROR] Failed to submit transcript to Cloud Run:", cloudErr);
          }
      }
